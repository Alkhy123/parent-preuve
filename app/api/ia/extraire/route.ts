// src/app/api/ia/extraire/route.ts
// Route serveur : description libre d'un jugement -> JSON "règles"
// (pension + frais + DVH + décision). Clé Mistral côté serveur. L'IA propose ;
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!reponse.ok) {
      return Response.json({ erreur: "Le service Mistral a renvoyé une erreur." }, { status: 502 });
    }
    data = await reponse.json();
  } catch {
    return Response.json({ erreur: "Impossible de joindre le service Mistral." }, { status: 502 });
  }

  const contenu = data?.choices?.[0]?.message?.content;
  if (typeof contenu !== "string") {
    return Response.json({ erreur: "Réponse Mistral inattendue." }, { status: 502 });
  }
  ...
  try {
    ...
    return Response.json({ erreur: "La réponse n'est pas un JSON valide." }, { status: 502 });
  }

  if (!structureValide(parsed)) {
    return Response.json({ erreur: "Le JSON renvoyé ne respecte pas le format attendu." }, { status: 502 });
  }
