...
export type MetaEtape = {
  /** Sous-titre affiche sous le titre dans la carte centrale. */
  objectif: string;
  /** Encart d'aide (bandeau doux) place sous le titre. */
  aide: string;
  /** Bloc "Pourquoi cette etape est utile ?" de la carte d'aide. */
  pourquoi: string;
  /** Conseil pratique (encart) dans la carte d'aide. */
  conseil: string;
...
export const META_ONBOARDING: Record<EtapeOnboarding, MetaEtape> = {
  ...
  "validation-regles": {
    ...
    conseil: "Relisez toujours les règles avant de valider.",
